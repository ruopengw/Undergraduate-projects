...
#include <OneWire.h> //温度传感器 DS18B20 头文件
#include <DallasTemperature.h> //温度传感器 DS18B20 头文件
...
void setup(){
 ...
 sensors.begin(); //初始化温度传感器 DS18B20
 //Time t(2015, 5, 16, 0, 2, 20, 1); rtc.time(t); //设置 DS1302 芯片初始时间
 set(rtc.year(), rtc.month(), rtc.date(), rtc.hour(), rtc.minutes(), rtc.seconds()); //通电读取DS1302 芯片的时间
 rtc.write_protect(false); // 关闭 DS1302 芯片写保护
...
void loop() {
 ...
 Clock_Alarm(); //闹钟时间蜂鸣
 GetTemperatures(); //获取 DS18B20 温度
 Temperatures_Alarm(); //超过指定温度报警
 Time t(year, month, day, hour, minute, second, 1); //断电将单片机的当前时间写到DS1302 芯片中
 rtc.time(t);
}
